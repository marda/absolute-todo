...
class TeamPresenter extends TodoBasePresenter
{
    ...
    private function _getTeamListRequest($idTodo)
    {
        if ($this->todoManager->canUserView($idTodo, $this->user->id))
        {
            $ret = $this->teamManager->getTodoList($idTodo);
            if (!$ret)
    ...
    private function _getTeamRequest($todoId, $teamId)
    {
        if ($this->todoManager->canUserView($todoId, $this->user->id))
        {
            $ret = $this->teamManager->getTodoItem($todoId, $teamId);
            if (!$ret)
                $this->httpResponse->setCode(Response::S404_NOT_FOUND);
            else
            {
    ...
    private function _postTeamRequest($urlId, $urlId2)
    {
        ...
        {
            $ret = $this->teamManager->teamTodoCreate($urlId, $urlId2);
            if (!$ret)
    ...
    private function _deleteTeamRequest($urlId, $urlId2)
    {
        if ($this->todoManager->canUserEdit($urlId, $this->user->id))
        {
            $ret = $this->teamManager->teamTodoDelete($urlId, $urlId2);
            if (!$ret)
                $this->httpResponse->setCode(Response::S404_NOT_FOUND);
            else
                $this->httpResponse->setCode(Response::S200_OK);
